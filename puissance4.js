(function ($) {
    ...
    $(".action").click(function () {
        ...
        $.fn.P4 = function () {
            ...
            $("body").css('background-image', 'url(assets/jeu.gif)');
            $('.audio').attr('src', 'assets/ymir.mp3');

            class Puissance4 {
                constructor(selector, Y, X, player1, player2) {//L'affichage des grilles pour les colonnes et lignes
                    this.Y = Y;
                    this.X = X;
                ...
                //Afficher le jeu
                drawGame() {
                    const $jeu = $(this.selector);//Ici sera stocker le jeu qui sera = à ce qu'on lui aura passé en paramètre lors de l'apl' de la classe
                    ...
                    for (let lgn = 0; lgn < this.Y; lgn++) {
                        ...
                        for (let col = 0; col < this.X; col++) {
                            const $col = $('<div>').addClass('col empty').attr("data-col", col).attr("data-lgn", lgn);//col pour nous c'est une div qui aura la class col empty et puis on stocke col et ca lgn;
                            $lgn.append($col);//Afficher cette colonne dans la lgn 
                        }
